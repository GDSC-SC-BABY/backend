...
public class BabyUpdateBean {
    ...
    public StateRes exec(String babyId, BabyUpdateReq babyUpdateReq) {
        if (babyRepository.findById(babyId).isPresent()) {
            BabyDAO babyDAO = babyRepository.findById(babyId).get();

            babyDAO.update(babyUpdateReq.getBabyId(), babyUpdateReq.getName(), babyUpdateReq.getGender(), babyUpdateReq.getDateTime(), babyUpdateReq.getImageUrl(), babyUpdateReq.getBirthHeight(), babyUpdateReq.getBirthWeight());
